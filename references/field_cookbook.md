...
> 本环境 `BD_ExpenseItem` / `BD_STAFF` 业务对象标识查不到，员工/费用项目用历史单据反查。员工速查：张三=123/BM03(财务部)，马菲=140/BM08(销售)，王爱丽=009/BM01或BM04。
...
- **更新分录用 `IsDeleteEntry="true"`**：不带 `FEntryID` 再 Save 一次会**追加**分录（2 行 → 4 行）；
  该开关只清分录、不动表头。
- 收票单是**手工建**时 `FGENERATETYPE` 为 `" "`（空），自动归集的是 `3`/`4`。二者在报销单侧无差别。
...
1. **收票单必须先在金蝶收票池里**。本环境「发票云自动归集」是开着的：数电票开票后次日凌晨
   自动生成收票单（批量，`FGENERATETYPE` = 3/4）。所以**发票号码能查到收票单**就能挂。
   查不到 = ① 纸票/未归集，需先用「发票助手」App 采集；② 号码错；③ 属其它组织。
...
工具：`recvin_link.py precheck <FID>`（体检）/ `submit <FID>`（有阻断项自动中止）/ `--force` 强提交。
