...
- ⚠️ 但随后再读，两张单都变成了 **`D`（重新审核）**，且 `APPROVERID` / `APPROVEDATE` 为空。
  对照：同环境另有一张单长期停在 `B`、两张是 `C` —— 说明 B/C 才是正常流转。
- `D` 的常见含义是**「被驳回 / 退回后待重新提交」**。**审批意见/流程节点在 UI 里看得到、
  WebAPI 读不到**，所以遇到 `D` 应向用户确认，不要自行重提交。
- 状态口径参考：`A` 创建 / `B` 审核中 / `C` 已审核 / `D` 重新审核 / `Z` 暂存
