...
### 🔑 那为什么"某组织要走附件"是对的？—— 是**业务做法**，不是系统限制
历史单据实测（2026-09-15）：

| 报销单 | 组织 | 状态 | 收票信息行数 |
|---|---|---|---|
| 历史单A | **105 示例二科技** | C 已审核 | **0 行** |
| 历史单B | 104 示例四科技 | B 审核中 | 1 行 |
| 历史单C | 101 示例科技 | C 已审核 | 2 行 |
...
### 📌 提交后的状态流转：跨组织单**提交会成功，但会被驳回**（实测关键结论）
...
- **全环境基线**（取最近 300 张报销单）：`C`×297、`B`×1（历史单B 长期卡住）、
  `D`×2（正好就是本次跨组织测试的 101720 / 101721）。
  → **跨组织挂票是本环境唯一出现 `D` 的单**，其余走本组织自己的票都能到 `C`。

**所以真正的风险落点不是「提交失败」，而是「提交成功后审批被驳回」**：
- WebAPI 层面的硬校验（金额 ≤ 发票价税合计、联动表合计、往来单位）**全部通过**，
  `Submit` 照常回 `IsSuccess=true` —— **接口不会替你挡这个错**。
- 驳回意见 / 流程节点**只在 UI 里看得到，WebAPI 读不到**（`WF_*` / `BOS_OperationLog`
  等表单均不存在，别去猜）。
- 因此遇到 `D` **必须让用户去 UI 看驳回意见**，**不要自行重提交**、更不要靠重试硬撞。

**推论（写进流程）**：`跨组织 + 发票购方 ≠ 报销组织` 这个组合，
在「🔴 强制确认节点」那里不是形式主义 —— 不确认就提交，**后果就是一张被驳回的单**。
...
2. **收票单上的联查号可能是「悬空号」**：`SPD00008634` 的 `FLINKIVNUMBER = CLFBX00000001`，
   ...
   别把它当成"这张票已经被用掉了"而误判。
   （反查单据用 `ExecuteBillQuery` + `FBillNo='...'` 精确匹配，
   返回空数组 `[]` 就是不存在，不会报错。）
